...
function TimelinePage(): ReactNode {
  ...
  const years = useMemo(
    () => Array.from(new Set(sorted.map((e) => new Date(e.date).getFullYear()))).sort(),
    [sorted],
  );
      ...
      <div className="mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mb-10">
          <Select value={type} onValueChange={(v) => setType(v as (typeof TYPES)[number])}>
            <SelectTrigger className="w-44" aria-label={t('timeline.filterType')}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {TYPES.map((tp) => (
          ...
          <Select value={year} onValueChange={setYear}>
            <SelectTrigger className="w-32" aria-label={t('timeline.filterYear')}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="relative">
            <div
              className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-border md:-translate-x-1/2"
              aria-hidden
            />
            <ol className="space-y-10">
              {filtered.map((e, i) => {
                ...
                return (
                  <li
                    key={e.id}
                    className={`relative md:grid md:grid-cols-2 md:gap-8 ${onLeft ? '' : 'md:[&>*:first-child]:order-2'}`}
                  >
                    <span
                      className="absolute left-4 md:left-1/2 top-6 grid h-8 w-8 place-items-center rounded-full gradient-primary text-base shadow-soft -translate-x-1/2 z-10"
                      aria-hidden
                    >
                      {meta.icon}
                    </span>
                    <div
                      className={`pl-12 md:pl-0 ${onLeft ? 'md:pr-12 md:text-right' : 'md:pl-12'}`}
                    >
                      <Card className="p-5 hover:shadow-elegant transition-shadow">
                        <div
                          className={`flex items-center gap-2 mb-2 ${onLeft ? 'md:justify-end' : ''}`}
                        >
                        ...
                        {e.attachments.youtubeUrl && (
                          <div className="mt-3 inline-flex items-center gap-1 rounded-md bg-destructive/10 text-destructive px-2 py-1 text-xs">
                            ▶ Vídeo do YouTube
                          </div>
                        )}
                        {e.memberIds.length > 0 && (
                          <div
                            className={`mt-3 flex flex-wrap gap-1.5 ${onLeft ? 'md:justify-end' : ''}`}
                          >
                        ...
                        <div
                          className={`mt-3 flex gap-3 text-xs text-muted-foreground ${onLeft ? 'md:justify-end' : ''}`}
                        >
