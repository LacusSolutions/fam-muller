...
function relTime(iso: string, t: (k: string, opts?: any) => string) {
  ...
  return (
    <Card className="p-5">
      <header className="flex items-center gap-3">
        ...
        <div>
          <Link to="/membros/$id" params={{ id: post.authorId }} className="font-semibold hover:text-primary">
            {author?.fullName}
          </Link>
      ...
      {post.images.length > 0 && (
        <div className={`mt-3 grid gap-1.5 ${post.images.length === 1 ? "" : "grid-cols-2"}`}>
          {post.images.slice(0, 4).map((img, i) => (
            <img key={i} src={img} alt="" loading="lazy" className="rounded-lg w-full aspect-video object-cover" />
          ))}
        </div>
      )}
      {post.youtubeUrl && (
        <a href={post.youtubeUrl} target="_blank" rel="noreferrer" className="mt-3 flex items-center gap-2 rounded-lg bg-destructive/5 border border-destructive/20 p-3 hover:bg-destructive/10">
          <span className="grid h-10 w-10 place-items-center rounded-full bg-destructive text-white">▶</span>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
        <div className="flex items-center gap-1">
          {REACTIONS.map((r) => (
            ...
            >
              <span aria-hidden>{r.emoji}</span> <span className="text-xs text-muted-foreground">{post.reactions[r.key]}</span>
            </button>
          ))}
        </div>
        <span className="text-xs text-muted-foreground">{totalReactions} • {post.comments.length} {t("timeline.comments")}</span>
      </div>
      ...
      <div className="mt-3 space-y-2">
        ...
        {post.comments.length > 2 && (
          <button onClick={() => setShowAll((v) => !v)} className="text-xs text-primary hover:underline">
            {showAll ? t("feed.hideComments") : t("feed.showComments", { count: post.comments.length })}
          </button>
        )}
        <form
        ...
        >
          <Input value={comment} onChange={(e) => setComment(e.target.value)} placeholder={t("feed.commentPlaceholder")} className="text-sm" aria-label={t("feed.commentPlaceholder")} />
          <Button type="submit" size="sm" variant="outline">{t("feed.sendComment")}</Button>
        </form>
      </div>
    </Card>
  );
...
function FeedPage() {
  ...
  const featured = [...posts]
    .reduce<Record<string, number>>((acc, p) => ({ ...acc, [p.authorId]: (acc[p.authorId] ?? 0) + 1 + p.comments.length }), {});
    ...
    <div className="mx-auto max-w-6xl px-4 md:px-6 py-10 grid gap-8 lg:grid-cols-[1fr_320px]">
      <section>
        ...
        <Card className="p-4 mb-6">
          <Textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={t("feed.placeholder")} rows={3} className="resize-none border-0 focus-visible:ring-0 p-0" />
          <div className="mt-3 flex justify-end">
            <Button onClick={publish} className="gradient-primary" disabled={!draft.trim()}>{t("feed.publish")}</Button>
          </div>
        </Card>
        ...
        <Card className="p-5">
          ...
          <ul className="space-y-3">
            {top3.map(([id]) => {
              ...
              return m ? (
                <li key={id}>
                  <Link to="/membros/$id" params={{ id }} className="flex items-center gap-3 hover:bg-surface-alt rounded-md p-1 -m-1">
              ...
              {upcoming.slice(0, 4).map(({ m, date, days }) => (
                <li key={m.id} className="flex items-center gap-3">
                  ...
                  <div className="text-sm">
                    <Link to="/membros/$id" params={{ id: m.id }} className="font-medium hover:text-primary">{m.fullName}</Link>
                    <p className="text-xs text-muted-foreground">{date.toLocaleDateString("pt-BR", { day: "2-digit", month: "long" })} • em {days}d</p>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <ul className="space-y-3">
            {recentEvents.map((e) => (
              <li key={e.id} className="text-sm">
                <span aria-hidden className="mr-1">{eventTypeMeta[e.type].icon}</span>
