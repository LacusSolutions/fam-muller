    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_180px_180px] mb-8">
        <div className="relative">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t("members.search")} className="pl-9" aria-label={t("members.search")} />
        </div>
        <Select value={gen} onValueChange={setGen}>
          <SelectTrigger aria-label={t("members.filterGeneration")}><SelectValue placeholder={t("members.filterGeneration")} /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={status} onValueChange={setStatus}>
          <SelectTrigger aria-label={t("members.filterStatus")}><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t("members.filterAll")}</SelectItem>
              ...
              <Link key={m.id} to="/membros/$id" params={{ id: m.id }} className="group">
                <Card className="overflow-hidden hover:shadow-elegant transition-all hover:-translate-y-1 h-full">
                  <div className="aspect-square overflow-hidden bg-surface-alt">
                    <img src={m.photo} alt={m.fullName} loading="lazy" className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
                  <div className="p-4">
                    <h3 className="font-display text-lg font-semibold leading-tight group-hover:text-primary">{m.fullName}</h3>
                    <p className="text-xs text-muted-foreground mt-1">{by} – {dy ?? "presente"}</p>
                    <Badge variant="secondary" className="mt-2 text-[10px]">{m.generation}ª {t("members.generation")}</Badge>
                  </div>
                </Card>
              </Link>
            );
