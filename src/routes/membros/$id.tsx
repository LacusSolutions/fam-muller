...
import { members, events, posts, getMember, eventTypeMeta } from "@/data/mockData";
...
export const Route = createFileRoute("/membros/$id")({
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-6 py-20 text-center">
      ...
      <Link to="/membros" className="mt-4 inline-block text-primary hover:underline">← Voltar</Link>
    </div>
  ),
...
function MiniCard({ id }: { id: string }) {
  ...
  if (!m) return null;
  return (
    <Link to="/membros/$id" params={{ id }} className="flex items-center gap-2 rounded-full bg-surface px-3 py-1.5 border border-border hover:border-primary transition-colors">
...
function MemberProfile() {
  ...
  const memberEvents = events.filter((e) => e.memberIds.includes(member.id)).sort((a, b) => +new Date(a.date) - +new Date(b.date));
  ...
  return (
    <article className="mx-auto max-w-5xl px-6 py-10">
      <Link to="/membros" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary mb-6">
      ...
      <header className="grid gap-8 md:grid-cols-[auto_1fr] items-start">
        <div className="relative mx-auto md:mx-0">
          <div className="rounded-full p-1.5 gradient-primary">
            <img src={member.photo} alt={member.fullName} className="h-44 w-44 md:h-52 md:w-52 rounded-full object-cover border-4 border-background" />
          </div>
        </div>
        <div>
          <Badge variant={member.isActive ? "default" : "secondary"} className={member.isActive ? "bg-primary" : "bg-accent/30 text-secondary"}>
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
            <span>{t("members.born")}: {new Date(member.birthDate).toLocaleDateString("pt-BR")}</span>
            {member.deathDate && <span>{t("members.died")}: {new Date(member.deathDate).toLocaleDateString("pt-BR")}</span>}
            <span>{member.generation}ª {t("members.generation")}</span>
          </div>
      ...
      <section className="mt-12">
        ...
        <Card className="p-6 space-y-6">
          {ancestors.length > 0 && (
            <div>
              <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">{t("members.ancestors")}</p>
              <div className="flex flex-wrap items-center gap-2">
                {ancestors.map((a) => <MiniCard key={a.id} id={a.id} />)}
                <ChevronRight className="h-4 w-4 text-muted-foreground" aria-hidden />
                <span className="text-sm font-semibold text-primary">{member.fullName.split(" ")[0]}</span>
              </div>
            </div>
          )}
          {spouse && (
            <div>
              <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">{t("members.spouse")}</p>
              <MiniCard id={spouse.id} />
            </div>
          )}
          {descendants.length > 0 && (
            <div>
              <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">{t("members.descendants")}</p>
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm font-semibold text-primary">{member.fullName.split(" ")[0]}</span>
                <ChevronRight className="h-4 w-4 text-muted-foreground" aria-hidden />
                {descendants.map((d) => <MiniCard key={d.id} id={d.id} />)}
              </div>
            </div>
          )}
        </Card>
      ...
      {/* Personal timeline */}
      <section className="mt-12">
        <h2 className="font-display text-2xl font-semibold mb-4">{t("members.personalTimeline")}</h2>
          ...
          <ol className="relative border-l-2 border-border ml-4 space-y-6">
            {memberEvents.map((e) => (
              <li key={e.id} className="pl-6 relative">
                <span className="absolute -left-[11px] top-1 grid h-5 w-5 place-items-center rounded-full bg-primary text-[10px]" aria-hidden>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {memberPosts.map((p) => (
              <Card key={p.id} className="p-5">
                <p className="text-xs text-muted-foreground mb-2">{new Date(p.createdAt).toLocaleDateString("pt-BR")}</p>
