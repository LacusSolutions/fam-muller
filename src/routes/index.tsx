...
import { events, eventTypeMeta, galleryImages, getMember, members, posts } from '@/data/mockData';
...
function useCountUp(
  target: number,
  duration = 1500,
): { ref: RefObject<HTMLDivElement | null>; v: number } {
...
function Stat({ icon, value, label }: { icon: string; label: string; value: number }): ReactNode {
...
function HomePage(): ReactNode {
  ...
  const totalPhotos =
    galleryImages.length + events.reduce((s, e) => s + e.attachments.photos.length, 0);
    ...
    <>
      ...
      <section className="relative h-[88vh] min-h-[560px] w-full overflow-hidden">
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center text-white">
          ...
          <h1
            className="font-display text-5xl md:text-7xl lg:text-8xl font-bold animate-fade-in-up"
          ...
          >
            <Button asChild size="lg" className="gradient-primary border-0 text-base">
              <Link to="/membros">
                {t('home.ctaMembers')} <ArrowRight className="ml-1.5 h-4 w-4" />
              </Link>
            </Button>
            <Button
              asChild
              size="lg"
              variant="outline"
              className="bg-white/10 backdrop-blur text-white border-white/30 hover:bg-white/20 hover:text-white text-base"
            >
      ...
      <section className="py-20 md:py-28 bg-background">
        <div className="mx-auto max-w-6xl px-6 grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative">
            ...
            <img
              src={aboutImg}
              alt="Foto histórica da família Müller"
              loading="lazy"
      ...
      <section className="py-20 bg-primary text-primary-foreground">
        <div className="mx-auto max-w-5xl px-6">
          <h2 className="font-display text-3xl md:text-4xl font-bold text-center mb-12 text-primary-foreground">
            {t('home.statsTitle')}
          </h2>
      ...
      <section className="py-20 bg-background">
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex items-end justify-between mb-8">
            ...
            <Link to="/linha-do-tempo" className="text-sm font-medium text-primary hover:underline">
              {t('home.seeAll')} →
            </Link>
          </div>
              ...
              .map((e) => (
                <Card key={e.id} className="p-6 hover:shadow-elegant transition-shadow">
                  <div className="flex items-center gap-2 mb-3">
                    <span
                      className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs ${eventTypeMeta[e.type].color}`}
                    >
