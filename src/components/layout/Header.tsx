...
import { Menu, X, Plus, Globe, TreePine } from "lucide-react";
...
export function Header({ onAddEvent }: { onAddEvent?: () => void }) {
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? "text-primary bg-primary/8" : "text-foreground/70 hover:text-foreground hover:bg-surface-alt"
                )}
              >
                {l.label}
              </Link>
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" aria-label={t("language.select")} className="gap-1.5">
            ...
            <DropdownMenuContent align="end">
              {langs.map((l) => (
                <DropdownMenuItem key={l.code} onClick={() => i18n.changeLanguage(l.code)}>
                  <span className="mr-2" aria-hidden>{l.flag}</span> {t(l.labelKey)}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          {onAddEvent && (
            <Button onClick={onAddEvent} size="sm" className="hidden sm:inline-flex gradient-primary">
      ...
      {open && (
        <div className="lg:hidden border-t border-border bg-background">
          <nav aria-label="Mobile" className="flex flex-col p-4 gap-1">
            ...
            {onAddEvent && (
              <Button onClick={() => { setOpen(false); onAddEvent(); }} className="mt-2 gradient-primary">
